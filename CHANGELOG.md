...
### Technical
- Bumped TOC `## Interface` to `120007` for Patch 12.0.7 — clears the "out of date" addon prompt

## [1.1.15] - 2026-03-26

### Fixed
- **Currency bar not updating**: Currency counts (e.g. Shard of Dundun) now reliably refresh after gaining currency while the bar was hidden due to combat, instance, or group settings. Also added a slight delay to avoid reading stale API data, and registered `CHAT_MSG_CURRENCY` as a backup event trigger

## [1.1.14] - 2026-03-25

### Fixed
- **Utility bar cooldown taint error**: Wrapped cooldown comparisons in `pcall` to handle secret/tainted values from Blizzard's `C_Spell.GetSpellCooldown` API after the latest mini-patch tightened taint enforcement. Cooldowns gracefully degrade (hide) instead of throwing errors

### Technical
- Added `## Category: Professions` to TOC metadata
